...
df = df[[
         ...
         # 'chufa_zuigao_wendu', 'chufa_zuidi_wendu',
         # 'daoda_zuigao_wendu', 'daoda_zuidi_wendu',
...
model2 = DecisionTreeClassifier(criterion='entropy',max_depth=8,max_features='sqrt',random_state=100)
...
model3 = GradientBoostingClassifier(n_estimators=15,max_depth=3,max_features=10)
...
#每隔100取一个点
fpr_micro1111 = []
for i in range(0,len(fpr_micro4),2000):
...
tpr_micro1111 = []
for i in range(0,len(tpr_micro4),2000):
...
model5=LogisticRegression(penalty="l2",solver="liblinear",tol=0.103)  #=0.00000001
...
#每隔100取一个点
fpr_micro11111 = []
for i in range(0,len(fpr_micro5),2000):
...
tpr_micro11111 = []
for i in range(0,len(tpr_micro5),2000):
...
plt.savefig("D:\Desktop/xiazai/ri/10特征ROC图.svg", dpi=3000,format="svg",bbox_inches='tight', pad_inches=0)
plt.show()
