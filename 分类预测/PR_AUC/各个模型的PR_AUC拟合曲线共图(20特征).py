...
model = RandomForestClassifier(n_estimators= 120, max_depth=4, min_samples_split=10,
                                min_samples_leaf=4,max_features=20, random_state=42)
...
#####################################################################################################################
##每隔100取一个点
precision1 = [0,0.005,0.008,0.01,0.02,0.1]
for i in range(0,len(precision),80):
    precision1.append(precision[i])
precision1.append(precision[-1])
# fpr_micro1.append(1.1)
print(precision1)

recall1 = [0.8,0.8,0.8,0.8,0.78,0.72]
for i in range(0,len(recall),80):
...
model2 = DecisionTreeClassifier(criterion='gini',max_depth=4,random_state=100)
...
##每隔100取一个点
precision22 = []
for i in range(0,len(precision2),5):
...
recall22 = []
for i in range(0,len(recall2),5):
...
model3 = GradientBoostingClassifier(n_estimators=10,max_depth=5,learning_rate=0.1,max_features=20)
...
###################
##每隔100取一个点
precision44 = [0,0.002,0.004,0.006,0.008,0.01,0.012,0.02,0.1]
...
recall44 = [0.8,0.8,0.8,0.8,0.8,0.8,0.8,0.78,0.72]
...
model5=LogisticRegression(penalty="l2",solver="liblinear",C=0.0003,random_state=42)
...
plt.savefig("D:\Desktop/xiazai/ri/20特征PR图.svg", dpi=3000,format="svg",bbox_inches='tight', pad_inches=0)
plt.show()
