...
def forcast():
    ...
    for idx in tqdm(range(n_route)):
        ...
        model = MultiOutputRegressor(KNN(n_neighbors=16))
        ...
        '''测试集上测试'''
        X_test, ground_truth = seq_test[:, :source_frame, idx, 0], seq_test[:, source_frame:source_frame + target_frame,
                                                                   idx, 0]
        # print(seq_test.shape)
        pred = model.predict(X_test)
        # print(ground_truth.shape, pred.shape)
