...
def sym_adj(adj):
    ...
    return adj.dot(d_mat_inv_sqrt).transpose().dot(d_mat_inv_sqrt).astype(
        np.float32).todense()  # (度矩阵^-1/2)*邻接矩阵*(度矩阵^-1/2)
...
def load_adj(pkl_filename, adjtype):
    ...
    if adjtype == "scalap":
        ...
        adj = [asym_adj(adj_mx), asym_adj(
            np.transpose(adj_mx))]  # 返回的是 [D^-1/2*A*D^-1/2, D_1^-1/2*A_1*D_1^-1/2], A_1 为 A的转置, JiNan数据集里邻接矩阵不是对称矩阵
...
def pems_load_dataset(dataset_dir, batch_size, n_route=228, n_his=12, n_frame=15, day_slot=288, valid_batch_size=None,
...
def zhengzhou_load_dataset(dataset_dir, batch_size, n_route=None, n_his=12, n_frame=15, day_slot=100,
                           valid_batch_size=None, test_batch_size=None):
    # 数据集划分, train,val,test所占的天数
    n_train, n_val, n_test = 8, 1, 1   # 5,1,1                        #数据集中共504行数据，504/7 = 72，即test占72行
    # generate training, validation and test data
    try:
        data_seq = pd.read_csv(dataset_dir, header=None).values
    except FileNotFoundError:
...
def zhengzhou_load_dataset_transition(dataset_dir, tpl_dir, batch_size, n_route=None, n_his=12, n_frame=15, day_slot=72,
                                      valid_batch_size=None, test_batch_size=None):
    # 数据集划分, train,val,test所占的天数
...
def exp_dataloader(dataset_dir):
    try:
        ...
        print(f'ERROR: input file was not found in {dataset_dir}.')
    # 数据集划分, train,val,test所占的天数
    n_train, n_val, n_test = 8, 1, 1  # 5, 1, 1
    day_slot = 200  # 72
    '''统计信息只能是训练集和验证集的数据计算'''
    x_stats = {'mean': np.mean(data_seq[:(n_train + n_val) * day_slot]),
               'std': np.std(data_seq[:(n_train + n_val) * day_slot])}
    ...
    '''数据是12->1'''
    tmp_seq = np.zeros((200, 10 + 1, 40, 1))  # np.zeros((72, 12+1, 716, 1))

    offset = 200 * 9 - 10  # 72 * 6 -12
    for i in range(200):  # 72
        sta = offset + i
        tmp_seq[i, :, :, :] = data_seq[sta:sta + 11].reshape(11, 40, 1)  # data_seq[sta:sta+13].reshape(13, 716, 1)

    ex_test = z_score(tmp_seq[1:], x_stats['mean'], x_stats['std'])
    data = {}
    '''这里需要用归一化的数据作为输入'''
    data['ex_x'], data['ex_y'] = ex_test[:, :10, :, :], tmp_seq[:, 10:, :, :]
    data['ex_loader'] = DataLoader(data['ex_x'], data['ex_y'], 200)
    return data, x_stats
...
def exp_dataloader_transition(dataset_dir, tpl_dir):
    ...
    TPL = np.array(TPL)
    ex_TPL = np.zeros((72, 12 + 1, 716, 10))  # np.zeros((72, 12+1, 716, 10))
    ...
    '''数据是12->1'''
    tmp_seq = np.zeros((72, 12 + 1, 716, 1))  # tmp_seq = np.zeros((72, 12+1, 716, 1))
...
def load_dataset_cluster(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    data['val_loader'] = DataLoader(data['x_val'], data['y_val'], valid_batch_size)
    data['test_loader'] = DataLoader(data['x_test'], data['y_test'], test_batch_size)
    data['train_loader_cluster'] = DataLoader_cluster(data['x_train'], data['y_train'], data['x_train_cluster'],
                                                      data['y_train_cluster'], batch_size)
    data['val_loader_cluster'] = DataLoader_cluster(data['x_val'], data['y_val'], data['x_val_cluster'],
                                                    data['y_val_cluster'], valid_batch_size)
    data['test_loader_cluster'] = DataLoader_cluster(data['x_test'], data['y_test'], data['x_test_cluster'],
                                                     data['y_test_cluster'], test_batch_size)
