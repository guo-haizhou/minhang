...
df = pd.read_csv('E:\meiguo2015\将数据集处理成邻接矩阵和特征矩阵\连通性邻接矩阵\meiguo2015_liantongxing40.csv')
print(df)
...
#         if data[i][j] == 0.0:
#             data[i][j] = data[i][j]  +  random.uniform(0,1)
print("处理后的data：")
print(data)
...
cmap = sns.heatmap(shuju,linewidths = 0.1,cmap=plt.get_cmap('winter_r'), annot=False, annot_kws={"fontsize": 10.5})
...
# cbar.set_label(label='连通性系数',fontsize=7.5)

#plt.savefig("E:\meiguo2015\将数据集处理成邻接矩阵和特征矩阵\连通性邻接矩阵/连通性邻接矩阵热图40.svg", dpi=300, format="svg", bbox_inches='tight', pad_inches=0)
# plt.savefig("E:\meiguo2015\一\图/连通性邻接矩阵热图402.svg", dpi=3000, format="svg", bbox_inches='tight', pad_inches=0)
plt.savefig("D:\Desktop\研作业\大/连通性邻接矩阵热图.svg", dpi=3000, format="svg", bbox_inches='tight', pad_inches=0)
plt.show()